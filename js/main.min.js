...
function createRipple(event) {
    ...
    }, 600); // Adjust based on your CSS animation duration
}
...
document.addEventListener('click', (e) => {
    ...
    if (closestButton) {
        ...
        const syntheticEvent = { ...e, currentTarget: closestButton };
        createRipple(syntheticEvent);
    }

    // Modal close (check if isDragging exists)
    if (target.classList.contains('modal') && (typeof isDragging === 'undefined' || !isDragging)) {
    ...
    if (downloadBtn) {
        ...
        setTimeout(() => {
            if (downloadBtn.classList.contains('btn-loading')) {
    ...
    if (viewBtn) {
        ...
        setTimeout(() => {
            viewBtn.innerHTML = '<span class="btn-icon">✅</span>Opening...';
            setTimeout(() => {
                if (viewBtn.innerHTML.includes('Opening...')) {
...
// Optional: Add touch support for mobile ripple effect
document.addEventListener('touchstart', (e) => {
    const target = e.target;
    const closestButton = target.closest('.btn-base, .cta-button, .project-link');

    if (closestButton && e.touches.length === 1) {
...
function openModal(modalId) {
    ...
    // Lazy init Swiper when modal opens
